...
### שלב 3: בניית קובץ המתקין (החלה → אימות → בנייה → אימות)
הריצו את פקודת ההידור:
```powershell
npm run dist
```
הפקודה הזו עושה **ארבעה** דברים, בסדר הזה: מחילה את מניפסט ההרשאה בזמן
הקומפילציה (`scripts/patch-nsis-template.js` — שורה אחת בתבנית של
electron-builder), מאמתת לפני הבנייה שהמנגנון בתוקף, בונה את המתקין
ב-electron-builder, ואז מאמתת את הקובץ שנבנה (`asInvoker`, בלי
`requireAdministrator`, וזהות לטביעת הבנייה).

למה ההחלה בזמן הקומפילציה ולא תיקון אחרי הבנייה: המתקין מופץ עם `asInvoker`
(בלעדיו הפעלה מתהליך שאינו מוגבר נכשלת **מיד** — שגיאה 740, בשקט), והמתקין
מרים את עצמו ב-`preInit` (`build/installer.nsh`). אבל **לתקן את המניפסט בתוך
ה-EXE אחרי הבנייה הורג את המתקין**: ל-NSIS יש בדיקת שלמות פנימית, וכל שינוי בית
בקובץ מפיל אותו — המתקין יצא מיד, בקוד 2, בלי חלון ובלי להתקין דבר (זה מה
שקרה למתקין של 1.7.2; פירוט ב-[MIGRATION-1.7.3.md](MIGRATION-1.7.3.md)).
...
> ⚠️ **אין לבנות עם `electron-builder --win` ישירות בלי ההחלה.** בנייה כזו
> מנפיקה מתקין עם `requireAdministrator`, ואז עדכון מתוך התוכנה ייכשל שוב בשקט
> בגרסאות ותיקות. `postinstall` מחיל את התבנית בכל `npm install` / `npm ci`,
> וקיימת בדיקה אוטומטית (`test/installer-manifest.test.js`) שנכשלת על ארטיפקט
> כזה ועל חזרה של תיקון-בדיעבד.
...
### שלב 8: אימות חי של נתיב העדכון (מומלץ אחרי כל פרסום)
מגיע לגיטהאב האמיתי ומוריד את המתקין שפורסם (~90MB), כדי לוודא שהשרשרת
סגורה מקצה לקצה — ולא רק שהקבצים תואמים זה לזה בגודל:
```powershell
npm run test:update-live
```
האימות מריץ את נתיב העדכון האמיתי של `main.js` ומצביע בין השאר על טביעה
שאינה תואמת: אם `version.json` מכריז על טביעה שאינה של הנכס שהועלה, ההתקנה
נעצרת והקובץ נמחק. אין צורך בהתקנה — הרצת המתקין ממוקה. זמן הריצה תלוי
ברשת (בין ~15 שניות ל-4 דקות, שתי הורדות).
